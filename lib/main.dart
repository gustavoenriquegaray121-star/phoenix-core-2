      ...
      body: SafeArea(child: Column(children: [
        ...
        Padding(
          ...
          child: Column(children: [
            AnimatedBuilder(
              animation: _pulse,
              builder: (_, __) => Text('PHOENIX CORE II',
                style: TextStyle(
                  color: cap.color,
                  fontSize: 22,
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            _ArrowBtn(
              ...
              color: _current < captains.length - 1
                  ? cap.color : Colors.white12,
...
// ─── Página individual de capitán ──────────────────────────────────────────
class _CaptainPage extends StatelessWidget {
  final CaptainData cap;
  final AnimationController pulse;
  const _CaptainPage({required this.cap, required this.pulse});
          ...
          child: Container(
            ...
            child: SingleChildScrollView(child: Column(
              ...
              children: [
                ...
                // Nombre + título
                Row(children: [
                  Text(cap.name, style: TextStyle(color: cap.color,
                ...
                ...[
                  ['DAÑO',      cap.damage / 1.8],
                  ['VELOCIDAD', cap.speed / 1.5],
                  ['CASCO',     cap.hullMax / 150],
                ].map((s) => Padding(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: ClipRRect(
                      ...
                      child: LinearProgressIndicator(
                        value: (s[1] as double).clamp(0.0, 1.0),
                        minHeight: 5,
...
// ═══════════════════════════════════════════════════════════════════════════
//  GAME SCREEN — Stage 1: Techo de Nave Alienígena
// ═══════════════════════════════════════════════════════════════════════════
...
class _PC2GameScreenState extends State<PC2GameScreen>
  ...
  // ── Posición de la mira (control táctil) ─────────────────────────────────
  Offset _aim = const Offset(0.5, 0.5); // normalizado 0..1

  // ── Proyectiles del jugador ───────────────────────────────────────────────
  final List<_Bullet> _bullets = [];
  double _shootCooldown = 0;

  // ── Enemigos básicos Stage 1 ──────────────────────────────────────────────
  ...
  void _updateBackground(double dt) {
    _spawnTimer -= dt;
    if (_spawnTimer <= 0) {
      _spawnTimer = 0.08 + _rng.nextDouble() * 0.25;
      _lines.add(_VHDLLineData(
        text: _frags[_rng.nextInt(_frags.length)],
        x: _rng.nextDouble(),
        speed: 0.05 + _rng.nextDouble() * 0.10,
        isCorrupt: _rng.nextDouble() < 0.18,
      ));
    }
    ...
    if (_glitchTimer <= 0) {
      _glitch = !_glitch;
      _glitchTimer = _glitch
          ? 0.05 + _rng.nextDouble() * 0.12
          : 2.0 + _rng.nextDouble() * 4.0;
    }
  }
  ...
  void _updateAbility(double dt) {
    if (!abilityActive) {
      abilityCharge = (abilityCharge + dt * 0.06).clamp(0, 1);
    } else {
      abilityTimer -= dt;
  ...
  void _updateShooting(double dt) {
    ...
    for (final b in _bullets) {
      b.y -= dt * 0.7; // sube hacia el objetivo
    }
  ...
  void _shoot(Offset tapNorm) {
    ...
    _bullets.add(_Bullet(
      x: 0.5,  // dispara desde el centro (nave del jugador abajo)
      y: 0.85,
      targetX: tapNorm.dx,
      targetY: tapNorm.dy,
  ...
  void _updateEnemies(double dt) {
    ...
    if (_enemySpawnTimer <= 0 && _enemiesKilled < 20) {
      _enemySpawnTimer = 1.2 - (score / 5000).clamp(0, 0.8);
      _enemies.add(_Enemy(
        x: 0.1 + _rng.nextDouble() * 0.8,
        y: -0.05,
        speed: 0.08 + _rng.nextDouble() * 0.06,
        hp: 2,
        type: _rng.nextInt(3),
      ));
    }
    ...
    for (final e in _enemies) {
      ...
      e.x += sin(e.y * 8 + e.phase) * dt * 0.04;
      e.x = e.x.clamp(0.05, 0.95);
      // Daño al jugador si llega abajo
      if (e.y > 1.0) {
    ...
    // Spawn boss cuando matas 20 enemigos
  ...
  void _updateBoss(double dt) {
    ...
    // Movimiento lateral
    b.moveTimer += dt;
    b.x += sin(b.moveTimer * 1.2) * dt * 0.12;
    b.x = b.x.clamp(0.15, 0.85);
    // Ataque del boss: dispara hacia el jugador
    b.attackTimer -= dt;
    if (b.attackTimer <= 0) {
      b.attackTimer = 2.0;
      b.projectiles.add(_BossProjectile(x: b.x, y: b.y + 0.08));
    }
    for (final p in b.projectiles) p.y += dt * 0.25;
    b.projectiles.removeWhere((p) {
      if (p.y > 1.0) return true;
      // Impacto en jugador (zona baja)
      if (p.y > 0.80 && (p.x - 0.5).abs() < 0.08) {
  ...
  void _checkCollisions() {
    ...
    for (final b in _bullets) {
      ...
      if (_boss != null && !_boss!.dead) {
        final bx = _boss!.x, by = _boss!.y;
        if ((b.x - bx).abs() < 0.12 && (b.y - by).abs() < 0.10) {
          _boss!.hp -= b.damage;
          toRemoveBullets.add(b);
          score += 10;
          if (_boss!.hp <= 0) {
            _boss!.dead = true;
            score += 5000;
            _explosions.add(_Explosion(x: bx, y: by, big: true));
          }
        }
      }
    }
    _bullets.removeWhere(toRemoveBullets.contains);
    _enemies.removeWhere(toRemoveEnemies.contains);
  }

  // ── Explosiones ───────────────────────────────────────────────────────────
  void _updateExplosions(double dt) {
    for (final e in _explosions) e.life -= dt * (e.big ? 0.5 : 1.2);
  ...
  static const _frags = [
    ...
    'ALIEN_CANNON_ONLINE;', 'HULL_BREACH_DETECTED;',
    'INTRUDER_ALERT_SECTOR_7;', 'DEFENSE_MATRIX_ARMED;',
  ];
  ...
  @override Widget build(BuildContext ctx) {
    ...
    return Scaffold(
      ...
      body: GestureDetector(
        // Toca en cualquier lugar = apunta y dispara
        onTapDown: (d) {
          final norm = Offset(
        ...
        child: Stack(children: [
          ...
          Positioned(
            ...
            child: CustomPaint(
              ...
              painter: _ShipPainter(color: cap.color,
                glow: abilityActive)),
          ),
          ...
          for (final e in _enemies)
            Positioned(
              left: e.x * size.width - 16,
              top: e.y * size.height - 16,
              child: CustomPaint(
                size: const Size(32, 32),
                painter: _EnemyPainter(type: e.type, hp: e.hp)),
            ),

          // ── Boss ──────────────────────────────────────────────────────
          if (_boss != null && !_boss!.dead) ...[
            Positioned(
              left: _boss!.x * size.width - 40,
              top: _boss!.y * size.height - 30,
              child: CustomPaint(
          ...
          if (_boss != null && _boss!.dead)
            Center(child: Column(
              ...
              Text('SECTOR LIMPIO', style: TextStyle(
                ...
                shadows: [Shadow(
                  color: cap.color.withOpacity(0.8), blurRadius: 20)])),
              const SizedBox(height: 8),
          ...
          for (final ex in _explosions)
            Positioned(
              left: ex.x * size.width - (ex.big ? 40 : 20),
              top: ex.y * size.height - (ex.big ? 40 : 20),
              child: CustomPaint(
                size: Size(ex.big ? 80 : 40, ex.big ? 80 : 40),
                painter: _ExplosionPainter(life: ex.life)),
            ),

          // ── Mira ──────────────────────────────────────────────────────
          Positioned(
            left: _aim.dx * size.width - 22,
  ...
  Widget _hullBar(CaptainData cap) => Column(
    ...
    SizedBox(width: 110, height: 5, child: ClipRRect(
      borderRadius: BorderRadius.circular(3),
      child: LinearProgressIndicator(
        value: hullIntegrity.clamp(0.0, 1.0),
        backgroundColor: Colors.white10,
        valueColor: AlwaysStoppedAnimation(
          hullIntegrity > 0.5 ? cIce :
  ...
  Widget _abilityBar(CaptainData cap) => Column(
    ...
    SizedBox(width: 80, height: 4, child: ClipRRect(
      ...
      child: LinearProgressIndicator(
        value: abilityCharge.clamp(0.0, 1.0),
        backgroundColor: Colors.white10,
        valueColor: AlwaysStoppedAnimation(
...
class _Bullet {
  double x, y;
  final double targetX, targetY, damage;
...
class _Enemy {
  ...
  _Enemy({required this.x, required this.y,
    required this.speed, required this.hp, required this.type});
}
...
class _Explosion {
  ...
  final bool big;
  double life;
  _Explosion({required this.x, required this.y,
    this.big = false}) : life = 1.0;
}
...
class _SkyPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size size) {
    // Fondo: espacio oscuro con tinte verde-azul (techo de nave)
    ...
    for (final l in lines) {
      ...
      final color = l.isCorrupt
          ...
          : Color.fromARGB((op * 150).toInt(), 0, 220, 100);
  ...
  @override void paint(Canvas canvas, Size s) {
    ...
    if (glow) {
      canvas.drawCircle(Offset(s.width / 2, s.height / 2),
        28, Paint()..color = color.withOpacity(0.15)..maskFilter =
          const MaskFilter.blur(BlurStyle.normal, 12));
    }
    final path = Path()
      ..moveTo(s.width / 2, 0)
      ..lineTo(s.width, s.height)
      ..lineTo(s.width / 2, s.height * 0.72)
      ..lineTo(0, s.height)
      ..close();
...
class _EnemyPainter extends CustomPainter {
  final int type; final double hp;
  const _EnemyPainter({required this.type, required this.hp});
  @override void paint(Canvas canvas, Size s) {
    final color = switch (type) {
      0 => const Color(0xFF44FF88),
      1 => const Color(0xFFFF8844),
      _ => const Color(0xFF8844FF),
    };
    ...
    if (type == 0) {
      ...
      // Cruz
      canvas.drawRect(
        Rect.fromLTWH(s.width * 0.3, 0, s.width * 0.4, s.height), p);
      canvas.drawRect(
        Rect.fromLTWH(0, s.height * 0.3, s.width, s.height * 0.4), p);
    }
    // Brillo
...
class _BossPainter extends CustomPainter {
  final double hp, maxHp; final bool active;
  const _BossPainter({required this.hp, required this.maxHp,
    required this.active});
  @override void paint(Canvas canvas, Size s) {
    final ratio = (hp / maxHp).clamp(0.0, 1.0);
    final color = Color.lerp(cRed, const Color(0xFFFF8800), ratio)!;
    // Cuerpo del boss (hexágono estilizado)
    final cx = s.width / 2, cy = s.height / 2;
    final path = Path();
    for (int i = 0; i < 6; i++) {
      final a = i * pi / 3 - pi / 6;
      final x = cx + cos(a) * 30, y = cy + sin(a) * 22;
    ...
    if (active) {
      canvas.drawCircle(Offset(cx, cy), 36,
        Paint()..color = cRed.withOpacity(0.2)
...
class _ExplosionPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size s) {
    final cx = s.width / 2, cy = s.height / 2;
    final r = s.width / 2 * (1 - life * 0.3);
    for (final color in [cFire, cGold, Colors.white]) {
      canvas.drawCircle(Offset(cx, cy), r,
        Paint()..color = color.withOpacity(life * 0.6)
